...
static void draw_eve_settings(reshade::api::effect_runtime* runtime) {
    ...
    float current_fps =
        static_cast<float>(Config::Manager::fps.first) / static_cast<float>(Config::Manager::fps.second);
    ...
    if (ImGui::IsItemHovered())
        ImGui::SetTooltip("Allow Rockstar Editor free camera to move without player-distance restriction");
    ...
    case DLL_PROCESS_ATTACH:
        try {
            ...
            static const wchar_t* k_ffmpeg_dlls[] = {
                L"avutil-58.dll",   L"avcodec-60.dll",   L"avformat-60.dll", L"avfilter-9.dll",
                L"avdevice-60.dll", L"swresample-4.dll", L"swscale-7.dll",
            };
